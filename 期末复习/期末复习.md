 ...
 * @LastEditTime: 2020-06-04 18:34:49
 * @License: CC-BY-NC-SA_V4_0 or any later version 
 -->
1. What is an OS?
   OS is a special layer.
   + 将复杂的硬件抽象成可以被统一使用的层面.
   + 保护共享资源,比如在rom里阻止进程互相影响.
   + 安全,权限问题,比如文件句柄,访问等等权限.某些功能只能在kernel mode提供,另一些只有在user model提供.防止互相乱用.
   + 在不同的逻辑已经之间的交流,比如HDD && RAM.

2. 更多的OS basics,virtual machine boundary.
...
3. OS basics: Program and process.
...
4. OS basics : Context swtich.
   + when a process do not be need, it is used up. then the process B will be load in memory.
   + move a to memory or os and load another process, switch process is expensive, because should load address space memory space and flush memory.

5. OS basics: scheduling, Protection.
...
6. OS basics: IO.
  + read data from storage or get input from keyboard must by IO.
...
8. Four Fundamental OS Concepts.
   ...
   + Address Space with with Translation, MMU( memory management unit)
...
11. 3rd Concepts: Process.
  ...
  + 进程间通信比进程内communication困难多了.
    + 进程内通信:信号量,事件,互斥量.
    + 进程间通信:管道(特别是pipe[]),信号,信号队列,共享内存,socket. 
...
14. Fork in action - finish.
  + use fork, father process frist stop action.
  ...
  + (详细的创建过程见 `\lab03_report02\overview.md`),很可能考.
...
18. Scheduling algorithms.
  ...
  SJF,FIFO等等算法都是需要了解的,建议参考`\midterm_2019\midterm_2019.md`.
...
21. 哲学家进餐问题,死锁.跳过一张图.
    死锁四条件
    + 资源被独占.
    + Hold and wait,Threads 拿着被别的Thread需要的资源,但是自己不会释放,就干等.
    + No preemption.
      + Resources are released only voluntarily by the thread holding the resource, after thread is finished with it.资源拿着不放手,主动被动都不放.
    + Circular wait,循环等待. >=2.
  
23. Binding of instructions and data to memory.
    + 把内存地址转换到物理地址.

24. Uniprogramming,单程序,直接用物理地址,爽到.
...
26. 地址转换这一块看`\lab09_report06\report06.md`后半部分,掠过.
PS:好怪,为啥是先讲页表再讲分页.
...
28. Demand paging.
  ...
  + 所以这里又需要置换算法
  + FIFO,MIN,LRU( use list优化),LRU优化之Second-change.
...
29. Transferriing data to/from controller.
  ...
  + DMA: direct memory access.
    ...
    + 这货过程请看`\report_08\report_08.md`,此处略.
...
31. Disk scheduling.
    ...
    期末考试里,只需要会计算序列,也就是说把算法记住.

32. FAT机制,file allocation table.
    1. 内部存储头,和size,头之后的都用链表实现.
    2. FAT内部的每个entry是32Bytes.
