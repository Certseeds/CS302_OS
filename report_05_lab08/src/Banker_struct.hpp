...
public:
    ...
    command(COMMAND_TYPE ct, int32_t pi, vector<int> res) : command_type(ct), process_id(pi),
...
public :
    ...
    explicit process(int32_t id, int n) : process_id(id), alloc_resource(vector<int32_t>(n)),
                                          max_resource(vector<int32_t>(n)),
                                          needed_resource(vector<int32_t>(n)) {};
